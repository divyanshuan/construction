...
const UploadHouseFileModal = ({ isOpen, onClose, house }) => {
  ...
  const fetchFiles = async () => {
    const res = await axios.get(`${baseUrl}/api/files/house/${house.house_id}`);
  ...
  const handleUpload = async () => {
    ...
    try {
      await axios.post(
        `${baseUrl}/api/files/house/${house.house_id}/upload`,
        formData,
        {
  ...
  return (
    <Modal
      ...
      title={`Manage Files - ${house?.title}`}
      big={true}
    >
...
export default UploadHouseFileModal;
